...
public final class DateUtil {
	...
	/**
	 * 取消当前时间的前一个小时开始时间 创建人：pananz 创建时间：2012-12-24 - 上午11:37:41
	 * 
	 * @return 返回类型：Date
	 * @exception throws
	 */
	...
	/**
	 * 取消当前时间的前一个小时结尾时间 创建人：pananz 创建时间：2012-12-24 - 上午11:38:07
	 * 
	 * @return 返回类型：Date
	 * @exception throws
	 */
	...
	public static List<Date> getDatesOfWeek() {
		String type = Global.LIVE_TYPE;
		List<Date> dates = new ArrayList<Date>();
		if ("1".equals(type) || StringUtil.isEmpty(type)) {
			// 本周
			Date firstDate = DateUtil.getFirstDayOfWeek();
			Date secondDate = DateUtil.addDay(firstDate, 1);
			Date threeDate = DateUtil.addDay(firstDate, 2);
			Date fourDate = DateUtil.addDay(firstDate, 3);
			Date fiveDate = DateUtil.addDay(firstDate, 4);
			Date sixDate = DateUtil.addDay(firstDate, 5);
			Date sevenDate = DateUtil.addDay(firstDate, 6);
			dates.add(firstDate);
			...
			// 下一周
			Date firstDate = DateUtil.getLastDayOfWeek();
			Date secondDate = DateUtil.addDay(firstDate, 1);
			Date threeDate = DateUtil.addDay(firstDate, 2);
			Date fourDate = DateUtil.addDay(firstDate, 3);
			Date fiveDate = DateUtil.addDay(firstDate, 4);
			...
			// 指定天数(当天的前n/2至后n/2时间段)
			int count = StringUtil.isEmpty(Global.LIVE_DATES) ? 3 : Integer
					.parseInt(Global.LIVE_DATES);
			Date today = new Date();
			for (int j = count; j > 0; j--) {
				Date date = DateUtil.addDay(today, -j);
				dates.add(date);
			}
