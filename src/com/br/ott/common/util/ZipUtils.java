...
public final class ZipUtils {
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	public static boolean unzipSingleFile(String zipFile, String fileName, String targetFile) {
	 ...
	 * <指定文件和文件夹列表压缩为一个文件 >
	 * 
	 * @param fileList 指定的文件和文件夹列表
	 * @param desFile 目标zip文件的绝对路径
	 * @return boolean 成功与否
	 * @exception throws [违例类型] [违例说明]
	 * @see [类、类#方法、类#成员]
	 */
	...
	private static void handleDir(String parentFolderPath, File file, Queue<String> allFiles, ZipOutputStream outZip) {
		File[] list = file.listFiles();
		// 如果没有子文件, 则添加进去即可
		if (list.length <= 0) {
			try {
				String path = getRelativePath(parentFolderPath, file.getAbsolutePath()) + File.separator;
	...
	private static void writeFileToZip(String parentFolderPath, File file, ZipOutputStream outZip) {
		ZipEntry zipEntry = new ZipEntry(getRelativePath(parentFolderPath, file.getAbsolutePath()));
		FileInputStream fis = null;
		try {
			outZip.putNextEntry(zipEntry);
			int len;
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	private static String getRelativePath(String parentFolderPath, String desFile) {
		return desFile.replace(parentFolderPath, "");
	}

	public static void main(String[] args) {
		String fullstr = "D:\\Program Files\\Tomcat 6.0\\webapps\\AppServer\\zips\\603443A313CB8DE9C749038B540BF186\\description.xml";
		String str = "D:\\Program Files\\Tomcat 6.0\\webapps\\AppServer\\zips\\603443A313CB8DE9C749038B540BF186\\";

		String endstr = fullstr.substring(fullstr.lastIndexOf("\\") + 1);
		String endstr1 = fullstr.replace(str, "");
		System.out.println(fullstr);
		System.out.println(str);
		System.out.println(endstr);
		System.out.println(endstr1);
	}
