...
public class FtpFileUtil {
	...
	public File downloadSingleFile(String targetBaseTmpPath, String downFileName)
			throws OTTException {
		// 登录FTP
		login();
		...
		try {
			...
			FTPFile[] files = ftpClient.listFiles();
			for (FTPFile ftpFile : files) {
				if (ftpFile.isFile() && ftpFile.getName().equals(downFileName)) {
					localFile = new File(targetBaseTmpPath + "/"
							+ ftpFile.getName());
					is = new FileOutputStream(localFile);
					boolean flag = ftpClient
	...
	public static void main(String[] args) {
		FtpFileUtil ffu = new FtpFileUtil(Global.FTP_HOSTNAME,
				Global.FTP_ACCOUNT, Global.FTP_PASSWD);
		try {
			boolean falg = ffu.upload("d:\\svnCheck.jar", "", "");
			System.out.println("flag:" + falg);
			File file = ffu.downloadSingleFile("d:/", "HD_20140119.txt");
