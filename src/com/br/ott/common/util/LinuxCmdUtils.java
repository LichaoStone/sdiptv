...
public class LinuxCmdUtils {
	 ...
	 * @exception throws
	 */
	public static boolean scpFile(String filePath, String finalPath,
			String dir, String scpUrl) {
		try {
			String sourcePath = Global.SOURCE_PATH + dir;
			String[] urls = scpUrl.split(",");
			if (StringUtil.isEmpty(scpUrl)) {
				logger.error("无传送地址........");
				return false;
			}
			for (String url : urls) {
				...
				String scpCmd = "scp " + filePath + " " + url + ":"
						+ sourcePath;
				String mvcmd = "ssh " + url + " mv " + filePath + " "
						+ finalPath;
