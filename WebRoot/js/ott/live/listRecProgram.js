$(function(){
	...
	$('#list-table td input[name="queue"]').change(function(e){
		  ...
		  if(reCat.test(queue) && queue.length < 5 && parseInt(queue) > 0){
			  var url =  window.ctx+ '/rec/changeDNSequence.do';
...
function delRecProgram($this, id){
	var url = url = window.ctx+ '/rec/delRecProgram.do?id='+id;
	if(!window.confirm("您确定要删除此推荐！")){
			return false;
		}
...
function updateRecStatus($this, id, status){
	var url = '';
	if(status =='0'){
		url = window.ctx+ '/rec/reverseRecProgram.do?id='+id;
		if(!window.confirm("您确定要启用此节目推荐！")){
			return false;
		}
	}else if(status == '1'){
		url = window.ctx+ '/rec/closeRecProgram.do?id='+id;
		if(!window.confirm("您确定要停用此节目推荐！")){
			return false;
		}
	}else{
		return;
