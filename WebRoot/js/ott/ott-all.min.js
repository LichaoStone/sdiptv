...
function changeModel(obj, dom) {
	var brandId=$('select#brandId option:selected').val();
	if (obj.value == ''|| brandId =='') {
	...
	$.get(window.ctx + "/equip/changeModel.do" , param,
			function(res) {
				if (res == null) {
					return;
				}
				...
				jQuery('#' + dom)
						.append("<option value=''>=\u8bf7\u9009\u62e9=</option>");
				for (var c in res) {
					jQuery("#" + dom).append("<option value='" + res[c].number
							+ "'>" + res[c].number + "</option>");
				}
			});
	return false;
...
function changeChannel(obj, dom){
	...
	$.get(window.ctx + "/rec/changeChannel.do" , param,
			function(res) {
				if (res == null) {
					return;
				}
				...
				for (var c in res) {
					...
					if(res[c].parentId=="0"){
						continue;
					}else if(res[c].parentId!="0"&& res[c].status=="0"){
						value="--"+res[c].name
					}else{
						value="----"+res[c].name
					}
